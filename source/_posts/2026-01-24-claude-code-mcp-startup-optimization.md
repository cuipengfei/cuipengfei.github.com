...
只要开了 `NONBLOCKING`，启动时间直接砍半，从 15 秒降到了 7 秒多。

而且我发现，单独改并发数（BATCH_SIZE）没用，因为只要是阻塞模式，并发再高也得等最慢的那个连完。只有开了非阻塞，并发数的调整才有意义——能让后台连接更快完成，让工具更早可用。

顺便提一句，既然 Anthropic 最近收购了 Bun，我还测了下用 Bun 运行的效果。本以为会秒杀 Node，结果启动速度反而慢了一丢丢（9.5s vs 7.6s）。不过内存占用确实低了很多，只有 Node 的三分之一。
...
现在打开 Claude Code，几乎是秒进界面。虽然刚才那几个 MCP 工具可能还在后台转圈圈，但至少我可以先敲下 "Hello" 了。
