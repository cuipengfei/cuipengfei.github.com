...
processWithGenericMethod type parameter: E
  First parameter type: E
  Second parameter type: java.util.List<? super E>
...
**重要补充**：但当结果被赋值给具体类型变量或进入增强 for 循环时，编译器仍会在使用点插入 `checkcast` 指令：
```java
Object obj = stringList.get(1);  // 获取时不转换
String s = obj;                  // 赋值给 String 变量时仍会插入 checkcast
```

## 编译对比：有无调试信息的差异

### 默认编译行为
- **Signature 属性**: 始终保留（与 -g 无关），是类型擦除的核心补偿机制
- **LocalVariableTypeTable**: 属于调试信息，默认构建通常包含调试信息从而可能保留；使用 `-g:none` 则被移除

### 无调试信息编译 (`javac -g:none`)
- 移除 LocalVariableTypeTable 和 LocalVariableTable
- 保留 Signature 属性
- 移除行号表等调试信息

**关键区别**：LocalVariableTable 仅含擦除后的局部变量类型与名字；LocalVariableTypeTable 携带泛型签名（需要调试信息）。
...
**关键发现**: Reified 的"魔法"实际上是编译时的内联展开，实际字节码中可见 `Intrinsics.reifiedOperationMarker(id, 'T')` 占位调用，其在内联时被替换为具体的 `instanceof`/`checkcast` 指令：

**重要说明**: `reifiedOperationMarker` 是编译器内部使用的占位符函数，在实际运行时永远不会被执行，仅在编译期被识别和替换为具体的类型操作指令。
...
val emptyIntArray = emptyArray<Int>()    // 生成: anewarray Integer (装箱的 Integer 数组)
val strings = mixedList.filterIsInstance<String>()  // 生成具体类型检查
```

**类型区分说明**：
```kotlin
emptyIntArray()       // 返回 IntArray (原始 int 数组，基于 JVM int[])
emptyArray<Int>()     // 返回 Array<Int> (装箱的 Integer 数组，基于 Integer[])
```
...
**核心发现**: Kotlin 标准库与第三方库中的 reified 函数都可以正常跨 JAR 调用！
...
**实现原理**:
- 标准库与第三方库一样，依赖 `@kotlin.Metadata` 中的内联函数体来跨 JAR 内联，无需源码
- Kotlin 编译器读取库字节码中的 Kotlin 元数据与内联体进行展开
- 编译时直接生成具体类型的字节码

**关键区别**:
- **arrayOf/emptyArray**: 在字节码层面直接生成 `anewarray` 具体类型指令，效果等同 reified 的类型具体化
- **第三方 reified**: 通过读取库中内联体元数据来实现类型具体化

### Java 调用 Kotlin Reified 的限制

**实验结果**: Java 无法调用真正的 reified 函数

**典型示例**：Java 无法调用真正的 reified 函数

```java
// 编译错误：真正的 reified 函数对 Java 不可见
kotlin.reflect.TypesJVMKt.typeOf(...);    // 无法调用
// 编译错误：arrayOfNulls 是 reified 函数
kotlin.ArraysKt.arrayOfNulls(5);          // 无法调用
// 编译错误：Jackson Kotlin 扩展的 reified 函数
mapper.readValue<Person>(json);           // 必须改用 readValue(json, Person.class)
```

**可调用的非 reified 替代版本**:
```java
// 正常工作：调用非 reified 的重载版本
List<String> list = kotlin.collections.CollectionsKt.listOf("hello", "world");
ObjectMapper mapper = new ObjectMapper();
Person person = mapper.readValue(json, Person.class);  // 传递 Class 参数
```

**根本原因**:
1. Java 编译器不理解 Kotlin 的 `inline` 语义和 `@kotlin.Metadata` 
2. 真正的 reified 函数在 Java 中不可见，只能看到方法存根
3. Kotlin 为互操作性提供了非 reified 的替代重载版本
...
**Project Valhalla** (Java 未来特性，目前尚未正式交付，具体时间表和特性可能调整) 可能会改变这一切：
...
**关键发现**：编译器为每个 reified 调用点生成了专门的内联类，每个调用点都会产生一个单独的合成匿名类：
...
```kotlin
// 每次调用都会生成独立的匿名类，不会共享
val person1 = mapper.readValue<Person>(json1)    // → $inlined$readValue$1  
val person2 = mapper.readValue<Person>(json2)    // → $inlined$readValue$2 (即使同类型)
val company = mapper.readValue<Company>(json3)   // → $inlined$readValue$3
val orgList = mapper.readValue<List<Person>>(json4) // → $inlined$readValue$4
```
...
- **基于元数据的内联**：Kotlin 编译器读取库字节码中的 Kotlin 元数据与内联体进行展开，无需源码文件
- **类型具体化**：为每个类型参数生成专门的匿名类（如 `TypeReference<Person>`）

#### 2. **与标准库的根本差异**
- **标准库**：某些 API（如 arrayOf）直接生成具体字节码指令（`anewarray String`），效果等同 reified 类型具体化
- **第三方库**：通过读取库中内联体元数据实现类型具体化，更符合标准 JVM 机制

#### 3. **实际限制**
- **元数据依赖**：第三方库必须携带 `@kotlin.Metadata` 注解和内联函数体信息
...
|---------|----------------|-------------------|
| **编译器支持** | 某些 API 有编译器内置支持 | 通用标记识别机制 |
| **字节码模式** | 直接类型指令 (`anewarray String`) | 内联类生成 (`TypeReference<T>`) |
| **工作原理** | 依赖 Kotlin 元数据内联展开 | 占位符替换协议 |
| **跨JAR实现** | 基于 @kotlin.Metadata | 元数据 + 标记系统 |
